...
export default function MagicalNightGarden({ active }: Props) {
  const hostRef = useRef<HTMLDivElement | null>(null);

  const markup = useMemo(() => {
    // Full garden markup copied from Flower-animation-main/FLORES.html,
    // excluding the background .night element (requested).
    return `
      <div class="gardenRoot ${active ? "active" : "paused"}">
        <div class="flowers">
  ...
  useEffect(() => {
    ...
    // Build DOM once; then update class for active/paused and scale.
    shadow.innerHTML = "";

    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = "/flowers/magical-flowers.css";
    shadow.appendChild(link);

    const style = document.createElement("style");
    style.textContent = `
      :host { display:block; width:100%; height:100%; position:relative; }
      /* Recreate the original demo's BODY layout inside our container (no background). */
      .gardenRoot {
        position:absolute;
        inset:0;
        display:flex;
      ...
      /* Scale the whole garden to fit the host box (the original uses vmin based on viewport). */
      .gardenRoot .flowers {
        position:relative;
        transform-origin: bottom center;
        transform: scale(calc(0.9 * var(--gardenScale, 1)));
        will-change: transform;
      }
      .gardenRoot.paused * { animation-play-state: paused !important; }
      .gardenRoot.active * { animation-play-state: running !important; }
      /* hide background from the original demo */
      .night { display:none !important; }
    `;
    shadow.appendChild(style);

    // Insert markup directly so .gardenRoot is a direct shadow child.
    const wrapper = document.createElement("div");
    wrapper.innerHTML = markup;
    shadow.appendChild(wrapper);

    const rootEl = shadow.querySelector(".gardenRoot") as HTMLElement | null;
    if (!rootEl) return;

    const updateScale = () => {
      const rect = host.getBoundingClientRect();
      const containerMin = Math.max(1, Math.min(rect.width, rect.height));
      const viewportMin = Math.max(1, Math.min(window.innerWidth, window.innerHeight));
      // Scale so vmin-based demo fits our container
      const s = Math.max(0.1, Math.min(2, containerMin / viewportMin));
      rootEl.style.setProperty("--gardenScale", String(s));
    };
